...
fn is_prime(n: f64) -> bool {
    ...
    match num {
        ...
        _ => {
            ...
            !(3..=limit).step_by(2).any(|i| num % i == 0)
        }
    }
}

fn handle_prime_request(request_str: &str, writer: &mut BufWriter<TcpStream>) -> std::io::Result<()> {
    let req: PrimeRequest = serde_json::from_str(request_str)?;
    println!("{:?}", req);

		if req.method != "isPrime".to_string() {
			return Err(Error::new(ErrorKind::InvalidData, "Invalid method"));
		}

    let resp = PrimeResponse::new(&req);
    writer.write_all(&serde_json::to_vec(&resp).expect("Couldn't serialize JSON to bytes")).expect("Couldn't write response to buffer");
